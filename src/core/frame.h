...
// leather add
public:
    std::vector<PtrMapMarkAruco> mvpMapMark; //相当于MapPoints
    std::shared_ptr<KeyFrameAruco> mpParent;
    Se2 dodo; //当前关键帧与mparent之间的测量
    std::map<int, Eigen::Matrix3d, std::less<int>,
            Eigen::aligned_allocator<std::pair<const int, Eigen::Matrix3d> > > mmapId2ArucoMsrInfo;
    std::map<int, Eigen::Matrix4d, std::less<int>,
            Eigen::aligned_allocator<std::pair<const int, Eigen::Matrix4d> > > mmapId2ArucoMsrPose;
    std::map<int, Eigen::Vector2d, std::less<int>,
            Eigen::aligned_allocator<std::pair<const int, Eigen::Vector2d> > > mmapId2ArucoMsrUV;
};
