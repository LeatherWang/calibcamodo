...
bool MeasurePoolPt3Kf2Mk::AddMsr(PtrMsrPt3Kf2Mk _pMsr) {
    ...
    msetMsr.insert(_pMsr);
    mmapKf2Msr.emplace(pKf, _pMsr); //为了方便通过关键帧得到PtrMsrPt3Kf2Mk集合
    mmapMk2Msr.emplace(pMk, _pMsr); //为了方便通过landmarker得到PtrMsrPt3Kf2Mk集合
    return true;
}

std::set<PtrMsrPt3Kf2Mk> MeasurePoolPt3Kf2Mk::GetMsrByKf(PtrKeyFrame _pKf) const {
    set<PtrMsrPt3Kf2Mk> setpMsrRet;
    auto iter_lowerbound = mmapKf2Msr.lower_bound(_pKf); //mmapKf2Msr的定义，参照上一个函数
...
std::set<PtrMsrPt3Kf2Mk> MeasurePoolPt3Kf2Mk::GetMsrByMk(PtrMapMark _pMk) const {
    set<PtrMsrPt3Kf2Mk> setpMsrRet;
    auto iter_lowerbound = mmapMk2Msr.lower_bound(_pMk); //mmapKf2Msr的定义，参照上上一个函数
