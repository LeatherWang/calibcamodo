...
void InitOptimizerSlam(Optimizer &opt, bool verbose){
    ...
    Algorithm* solver = new Algorithm(blockSolver); //Algorithm是typedef的g2o::OptimizationAlgorithmLevenberg;
...
g2o::ParameterCamera* AddParaCamera(Optimizer &opt, const cv::Mat& K, const g2o::Isometry3D& se3offset, int id) {
    ...
    para->setOffset(se3offset); /** 设置补偿->从相机坐标系挪到机器人坐标系下*/
...
void AddVertexSE2(Optimizer &opt, const g2o::SE2 &pose, int id, bool fixed) {
    ...
    /** @attention big bug dont use again*/
//    if(id == 1)
//        fixed = true;

    v->setFixed(fixed); //不固定!!
    v->setId(id);
    opt.addVertex(v);
...
/// leather add
void AddVertexSBAPointXYZ(Optimizer &opt, const g2o::Vector3D &xyz, int id, bool marginal){
    g2o::VertexSBAPointXYZ* v = new g2o::VertexSBAPointXYZ();
    v->setEstimate(xyz);
    v->setId(id);
    v->setMarginalized(marginal);
...
g2o::EdgeVSclam* AddEdgeVSclam(Optimizer &opt, int idKf, int idMp, int idCamOffset, int idCamParam,
    ...
    e->vertices()[0] = opt.vertex(idKf);
    e->vertices()[1] = opt.vertex(idMp);
    e->vertices()[2] = opt.vertex(idCamOffset); //设置顶点:外参
    e->setMeasurement(measure);
    e->setInformation(info);
    e->setParameterId(0, idCamParam); //设置相机内参
    opt.addEdge(e);
...
/// leather add
g2o::EdgeUV_SE3_XYZ* AddEdgeUV_SE3_XYZ(Optimizer &opt, int idKf, int idMp, int idCamParam,
                                   const g2o::Vector2D &measure, const g2o::Matrix2D &info) {
    g2o::EdgeUV_SE3_XYZ* e = new g2o::EdgeUV_SE3_XYZ();
    e->vertices()[0] = opt.vertex(idKf);
    e->vertices()[1] = opt.vertex(idMp);
    e->setMeasurement(measure);
    e->setInformation(info);
